...
if ($runningClients) {
    ...
    throw "Hurricane is running (PID: $processIds). Close the client before rebuilding client/bin. Replacing hafen.jar while Java is using it can kill the UI thread with NoClassDefFoundError and leave a frozen white window."
}
