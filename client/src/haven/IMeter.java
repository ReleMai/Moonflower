...
public class IMeter extends LayerMeter {
	...
	public static volatile HealthState lastHealthState;
	...
	public static final class HealthState {
		...
		public final double softPercentage;
		public final double hardPercentage;
		public final String displayText;

		public HealthState(int shp, int hhp, int mhp, double softPercentage, double hardPercentage, String displayText) {
			this.shp = shp;
			this.hhp = hhp;
			this.mhp = mhp;
...
//	    Tex bg = this.bg.get().flayer(Resource.imgc).tex();
		...
		if (msg == "tip") {
			...
			if (value.contains("/")) { // ND: this part removes the HHP, so I only show the SHP and MHP
				...
				if (hps.length == 4) { // ND: Sparring adds a 4th value
					sparring = true;
					updateHealthState(hps[0], hps[2], hps[3]);
					value = hps[0] + " / " + hps[hps.length - 1]; // ND: hps[0] is SHP, hps[1] is sparring hp, hps[2] is HHP, hps[3] is MHP
					characterCurrentHealth = value;
				} else {
					sparring = false;
					updateHealthState(hps[0], hps[1], hps[2]);
	...
	private static void updateHealthState(String shpText, String hhpText, String mhpText) {
		try {
			int shp = parseHealthValue(shpText);
			int hhp = parseHealthValue(hhpText);
			int mhp = parseHealthValue(mhpText);
			characterSoftHealthPercent = (mhp > 0) ? (shp * 100.0 / mhp) : 0;
			double hardPercentage = (mhp > 0) ? (hhp * 100.0 / mhp) : 0;
			lastHealthState = new HealthState(
					shp,
					hhp,
					mhp,
					characterSoftHealthPercent,
					hardPercentage,
					shp + " / " + mhp
			);
	...
	private static int parseHealthValue(String text) {
